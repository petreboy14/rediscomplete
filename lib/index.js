...
RedisComplete.prototype.update = function (options, cb) {
  ...
  this.conn.hget({key: this.app + ':' + ns + ':docs', field: data[idField]}, function (err, item) {
    err && cb(err);
    if (!err) {
      if (!item) {
        self.add({data: [data], ns: ns, idField: idField, complKey: complKey}, cb);
      } else {
        item = JSON.parse(item);
        if (item[complKey] !== data[complKey]) {
          self.remove({ ns: ns, id: data[idField]}, function (err) {
            err && cb(err);
            if (!err) {
              self.add(options, cb);
            }
          ...
          self.conn.hset({key: self.app + ':' + ns + ':docs', field: data[idField], value: data}, cb);
        }
      }
    }
  });
...
RedisComplete.prototype.remove = function (options, cb) {
  ...
  this.conn.hdel({key: this.app + ':' + ns + ':docs', field: id}, function (err) {
    err && cb(err);
    if (!err) {
