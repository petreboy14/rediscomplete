...
RedisComplete.prototype.update = function (options, cb) {
  ...
  async.each(data, function (dataItem, cb) {
    self.conn.hget({key: self.app + ':' + ns + ':docs', field: dataItem[idField]}, function (err, item) {
      ...
      if (!err) {
        if (!item) {
          self.add({data: [dataItem], ns: ns, idField: idField, complKey: complKey}, cb);
        } else {
          item = JSON.parse(item);
          if (item[complKey] !== dataItem[complKey]) {
            ...
            self.conn.hset({key: self.app + ':' + ns + ':docs', field: dataItem[idField], value: dataItem}, cb);
          }
        }
      }
    });
